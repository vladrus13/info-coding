...
void init_modules(const params &p, modules &m) {
    ...
    m.encoder = std::make_unique<module::Encoder_polar<>>(p.K, p.N, get_frozen());
    m.modem = std::make_unique<module::Modem_BPSK<>>(p.N);
    m.channel = std::make_unique<module::Channel_AWGN_LLR<>>(p.N, p.seed);
    m.decoder = std::make_unique<module::Decoder_repetition_std<>>(p.K, p.N);
